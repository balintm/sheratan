/// \file errhdl/test/lib_info_test.cpp
...
#include "sheratan/errhdl/lib_info.hpp"
...
BOOST_AUTO_TEST_SUITE(lib_info)
  ...
  {
    BOOST_CHECK_EQUAL(std::string(sheratan::errhdl::lib_name), std::string(SHERATAN_LIB_NAME));
  }

  /// \brief Unit-test case: Library version.
  BOOST_AUTO_TEST_CASE(lib_version)
  {
    BOOST_CHECK_EQUAL(std::string(sheratan::errhdl::lib_version), std::string(SHERATAN_LIB_VERSION));
  }

  /// \brief Unit-test case: Library version tag.
  BOOST_AUTO_TEST_CASE(lib_version_tag)
  {
    BOOST_CHECK_EQUAL(sheratan::errhdl::lib_version_tag, SHERATAN_LIB_VERSION_TAG);
  }
